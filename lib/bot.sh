...
# ---- config ----------------------------------------------------------------
# bot_write_config TOKEN CHAT_ID USER_ID [RELAY_AGENT]
bot_write_config() {
  local token="$1" chat="$2" user="$3" agent="${4:-}"
...
_bot_setup() {
  local unattended=0 token="" chat="" user="" agent=""
  while [[ $# -gt 0 ]]; do
    case "$1" in
  ...
  if [[ "$unattended" -eq 1 ]]; then
    ...
    [[ -n "$user" ]] || user="$chat"
    bot_write_config "$token" "$chat" "$user" "$agent"
    bot_install_sudoers "$agent"
    bot_install_service
    log "bot configured (unattended)"
    return 0
  fi
  ...
  bot_write_config "$tok" "$chat_id" "$user_id" "$agent"
  bot_install_sudoers "$agent"
  bot_install_service
